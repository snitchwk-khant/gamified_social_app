...
create table if not exists public.monthly_champions (
  ...
  constraint monthly_champions_month_key unique (month)
);

-- Existing installations of this migration may already have the table.
-- Add updated_at safely without touching created_at or existing data.
...
-- INSERT can introduce a new candidate for a month, so refresh immediately.
...
-- UPDATE only matters when ranking inputs change. Avoid recalculating champions
-- for unrelated updates such as timestamps or metadata changes.
...
-- The refresh function is used by database triggers and trusted backend jobs.
-- Normal authenticated clients can read monthly_champions, but must not call
-- this RPC directly.
